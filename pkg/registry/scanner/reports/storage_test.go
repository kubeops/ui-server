...
// TestAuthorizeScopes verifies that each CVEReport request scope is gated against the
// expected access before any cluster data is read. The authorizer denies, so Create
// returns Forbidden and we assert on the attributes it was asked about.
func TestAuthorizeScopes(t *testing.T) {
	cases := []struct {
		...
		{
			name:         "namespace",
			oi:           &kmapi.ObjectInfo{Resource: kmapi.ResourceID{Name: "namespaces"}, Ref: kmapi.ObjectReference{Name: "ns1"}},
			wantVerb:     "list",
			wantResource: "pods",
			wantNS:       "ns1",
		},
		{
			name:         "pod",
			oi:           &kmapi.ObjectInfo{Resource: kmapi.ResourceID{Name: "pods"}, Ref: kmapi.ObjectReference{Namespace: "ns2", Name: "pod1"}},
			wantVerb:     "get",
			wantResource: "pods",
			wantNS:       "ns2",
			wantName:     "pod1",
		...
		{
			name:         "object",
			oi:           &kmapi.ObjectInfo{Resource: kmapi.ResourceID{Group: "apps", Kind: "Deployment"}, Ref: kmapi.ObjectReference{Namespace: "ns3", Name: "dep1"}},
			wantVerb:     "get",
			wantAPIGroup: "apps",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			in := &reportsapi.CVEReport{}
			if tc.oi != nil {
				in.Request = &reportsapi.CVEReportRequest{ObjectInfo: *tc.oi}
			}
