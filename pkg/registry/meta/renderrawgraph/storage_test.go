...
// TestCreateDeniedForUnreadableSource verifies a source-scoped request requires "get" on
...
func TestCreateDeniedForUnreadableSource(t *testing.T) {
	...
	in := &rsapi.RenderRawGraph{
		Request: &rsapi.RenderRawGraphRequest{
...
// TestCreateDeniedForWholeGraph verifies the no-source whole-cluster graph requires
// cluster-wide read access (get on */*).
func TestCreateDeniedForWholeGraph(t *testing.T) {
	fa := &fakeAuthorizer{decision: authorizer.DecisionDeny}
	kc := fake.NewClientBuilder().WithRESTMapper(testMapper()).Build()
	r := NewStorage(kc, fa)

	in := &rsapi.RenderRawGraph{Request: &rsapi.RenderRawGraphRequest{}}
	_, err := r.Create(ctxWithUser(), in, nil, nil)
	if !apierrors.IsForbidden(err) {
		t.Fatalf("expected Forbidden, got %v", err)
	}
