...
const (
	ResourceCodePostgresRoute     = "pgroute"
	ResourceKindPostgresRoute     = "PostgresRoute"
	ResourceSingularPostgresRoute = "postgresroute"
	ResourcePluralPostgresRoute   = "postgresroutes"
)
...
// +kubebuilder:resource:path=postgresroutes,singular=postgresroute,shortName=pgroute,categories={route,appscode}
...
type PostgresRouteSpec struct {
	...
	//
	// If a hostname is specified by both the Listener and PostgresRoute, there
	// must be at least one intersecting hostname for the PostgresRoute to be
	// attached to the Listener. For example:
	//
	// * A Listener with `test.example.com` as the hostname matches PostgresRoutes
	//   that have either not specified any hostnames, or have specified at
	//   least one of `test.example.com` or `*.example.com`.
	// * A Listener with `*.example.com` as the hostname matches PostgresRoutes
	//   that have either not specified any hostnames or have specified at least
	//   one hostname that matches the Listener hostname. For example,
	//   `test.example.com` and `*.example.com` would both match. On the other
	//   hand, `example.com` and `test.example.net` would not match.
	//
	// If both the Listener and PostgresRoute have specified hostnames, any
	// PostgresRoute hostnames that do not match the Listener hostname MUST be
	// ignored. For example, if a Listener specified `*.example.com`, and the
	// PostgresRoute specified `test.example.com` and `test.example.net`,
	// `test.example.net` must not be considered for a match.
	//
	// If both the Listener and PostgresRoute have specified hostnames, and none
	// match with the criteria above, then the PostgresRoute is not accepted. The
...
func (r *PostgresRoute) CustomResourceDefinition() *apiextensions.CustomResourceDefinition {
	return crds.MustCustomResourceDefinition(GroupVersion.WithResource(ResourcePluralPostgresRoute))
}
